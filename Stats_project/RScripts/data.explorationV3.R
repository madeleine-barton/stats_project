...
nam_juldays<-nam.df%>%
  ...
  dplyr::select(-Admin_areas, -Planting_period_end, -Harvesting_period_onset, -Harvesting_period_end)
...
others_juldays<-cam_tza_df%>%
  ...
  dplyr::select(-Admin_areas, -Planting_period_onset, -Planting_period_end, -Harvesting_period_onset, -Harvesting_period_end)
...
df2<-df%>%
  unite(unite(Combo, Country, AEZ, sep="_", remove=FALSE) )%>%
  unite(unite(Combo_Crop, Country, AEZ, sep="_", remove=FALSE) )
colnames(df)
#specify more clearly whether it's a first or  second season crop
df$Add_Info


df1<-df%>%
  separate(Add_Info, into=c("water_source", "season_order"), sep=",", fill = "left")

write.csv(df, "C:/Users/bar823/Documents/MGB_docs/Data_School/stats_proj/Stats_project/Cleaned_Data/Cleaned_data_all.csv")
...
df1<-df%>%distinct(Country, AEZ,Crop, .keep_all = TRUE)%>%
...
Crop_diverse0<-left_join(df1, Country_zones, by="Country")
...
write.csv(Crop_diverse2,"C:/Users/bar823/Documents/MGB_docs/Data_School/stats_proj/Stats_project/Cleaned_Data/Data combined for diversity.csv" )


#ggplot of number of zones per country
df<-as.data.frame(unique(cbind(Crop_diverse1$Country, Crop_diverse1$Zone_number)))
colnames(df)<-c("Country", "Zone_number")
...
df<-as.data.frame(unique(cbind(Crop_diverse1$Country, Crop_diverse1$C_Diversity)))
colnames(df)<-c("Country", "C_Diversity")
ggplot(df, aes(x=Country, y=C_Diversity))+
  geom_bar(stat="identity", width = 0.4)+
...
#ggplot of proportion of the countries total crops in each zone 
ggplot(Crop_diverse2, aes(x=Country, y=prop_crops))+
  geom_boxplot() +
  labs(y="Poprotion of Countries Crops in each Zone")
...
#propotion of countries diversity within each zone
m1<-glm(prop_crops~Country, data=Crop_diverse2, family=poisson)
anova(m1)
summary(m1)
emmeans(m1, pairwise~Country)
plot(m1, which=1)
